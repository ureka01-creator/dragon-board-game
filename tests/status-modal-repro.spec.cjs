...
test.describe('iPhone hero status modal reproduction', () => {
  test('status button opens a visible, interactive status sheet after page scroll', async ({ page }, testInfo) => {
    test.skip(testInfo.project.name !== 'webkit-iphone', 'This regression reproduces the iPhone Safari path.');
    ...
    console.log('STATUS_MODAL_OPENED', JSON.stringify(opened));
  ...
  test('status modal survives repeated touch open/close without observer lockup', async ({ page }, testInfo) => {
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      const opened = await viewportSnapshot(page);
      console.log(`STATUS_MODAL_CYCLE_${i + 1}`, JSON.stringify(opened));
      expectCardInsideVisibleViewport(opened);
      await closeButton.tap();
