...
(() => {
  ...
  style.textContent = `
    ...
    .final-dungeon-d20{
      ...
      filter:drop-shadow(3px 5px 0 #090705)
    }
    ...
    .final-dungeon-guardian-clear{
      margin-top:10px;padding:11px;border:1px solid #556a42;background:#162012;color:#aec88b;font-size:11px;line-height:1.6
    }
    ...
    .final-dungeon-preserved-entry[hidden]{display:none!important}

    @media(max-width:520px){
      .final-dungeon-track{gap:3px;padding:9px 5px}.final-dungeon-track-node{width:32px;height:32px;font-size:16px}
      .final-dungeon-track-stage:not(:last-child)::after{top:16px}.final-dungeon-track-label{font-size:8px}.final-dungeon-stage-title{font-size:20px}
    }
  `;
  document.head.appendChild(style);

  let scheduled = false;
  let waitingCorridorCoreResult = false;
  ...
  function trackHTML(currentIndex) {
    ...
    return `<div class="final-dungeon-track" data-final-dungeon-stage="${current}" aria-label="최종 던전 진행도">${STAGES.map((stage,index) => {
      ...
      return `<div class="final-dungeon-track-stage ${cls}">
        <span class="final-dungeon-track-node">${index < current ? '✓' : stage.icon}</span>
  ...
  function clickCoreModalClose() {
    modalCloseBtn.hidden = false;
    modalCloseBtn.click();
  }

  function clickPreservedEntry(entryButton) {
    bypassEntryIntercept = true;
    try { entryButton.click(); }
    finally { bypassEntryIntercept = false; }
  }

  function showCorridorPrompt({ entryButton = null } = {}) {
    modalCloseBtn.hidden = true;
    corridorRelease = entryButton
      ? () => clickPreservedEntry(entryButton)
      : () => clickCoreModalClose();

    if (entryButton) {
      const sheet = entryButton.closest('.final-dungeon-sheet');
      if (sheet) {
        sheet.classList.add('final-dungeon-preserved-entry');
        sheet.hidden = true;
      }
    }

    const prompt = document.createElement('div');
    prompt.className = 'final-dungeon-stage-shell final-dungeon-corridor';
    prompt.dataset.finalDungeonUx = 'corridor-prompt';
    prompt.innerHTML = `
    ...
    if (!entryButton) modalContent.innerHTML = '';
    modalContent.appendChild(prompt);

    const dieEl = prompt.querySelector('[data-final-dungeon-d20]');
    const rollBtn = prompt.querySelector('.final-dungeon-roll-btn');
    rollBtn?.addEventListener('click', () => {
      ...
      waitingCorridorCoreResult = true;
      setTimeout(() => {
  ...
  function showCorridorResult(titleText, bodyText) {
    waitingCorridorCoreResult = false;
    ...
    modalContent.innerHTML = `
      <div class="final-dungeon-stage-shell final-dungeon-corridor" data-final-dungeon-ux="corridor-result">
        ...
        <div class="final-dungeon-roll-zone">
          ...
          <div class="final-dungeon-roll-result ${result.success ? 'success' : 'failed'}">
            <strong>${result.success ? 'SUCCESS' : 'FAILED'}</strong>
            ${Number.isFinite(result.total) ? `D20 ${result.die} + 보정 ${result.bonus} = ${result.total}` : result.text}<br>
            ${result.success ? '봉인석의 힘이 함정을 잠재웠다. 회랑 돌파 성공.' : `용염 함정이 폭발했다.${result.damage ? ` HP ${result.damage} 피해.` : ''} 그래도 회랑은 돌파했다.`}
          </div>
          <button type="button" class="pixel-btn primary final-dungeon-continue-btn">다음 구간으로 이동 ▶</button>
        </div>
      </div>`;
    modalContent.querySelector('.final-dungeon-continue-btn')?.addEventListener('click',clickCoreModalClose,{once:true});
  }
  ...
  function decorateGuardianClear() {
    ...
    if (!modalContent.querySelector('.final-dungeon-guardian-clear')) {
      modalContent.insertAdjacentHTML('beforeend','<div class="final-dungeon-guardian-clear">✓ 성문 수호자 격파 · 다음은 봉인의 회랑이다.</div>');
    }
  }
  ...
  function showThroneWarning({ entryButton = null } = {}) {
    ...
    if (entryButton) {
      const sheet = entryButton.closest('.final-dungeon-sheet');
      if (sheet) { sheet.classList.add('final-dungeon-preserved-entry'); sheet.hidden = true; }
    }
    modalContent.querySelector('.final-dungeon-throne-warning')?.remove();
    const warning = document.createElement('div');
    warning.className = 'final-dungeon-throne-warning';
    warning.innerHTML = `
    ...
    warning.querySelector('.final-dungeon-throne-open')?.addEventListener('click',() => {
      if (entryButton) clickPreservedEntry(entryButton);
      else clickCoreModalClose();
    },{once:true});
  }
  ...
  function enhanceCurrentModal() {
    ...
    const entry = modalContent.querySelector('.final-dungeon-sheet:not([hidden])');
    if (entry) { enhanceEntrySheet(entry); return; }
    if (modalContent.querySelector('[data-final-dungeon-ux="corridor-result"], [data-final-dungeon-ux="altar"], .final-dungeon-throne-warning')) return;

    const titleText = modalContent.querySelector('h3')?.textContent?.trim() || '';
    const bodyText = modalContent.querySelector('p')?.textContent?.trim() || '';
    if (titleText.includes('성문 돌파')) { decorateGuardianClear(); return; }
    if (waitingCorridorCoreResult && titleText.includes('봉인의 회랑')) { showCorridorResult(titleText,bodyText); return; }
    if (titleText.includes('용의 제단')) { enhanceAltar(bodyText); }
  }
  ...
  // Capture only the two beats that must be player-controlled before the core's
  // existing Promise resolves. Other buttons remain owned entirely by game.js.
