...
if (boardTitle) boardTitle.textContent = `${areaMeta?.icon || '🗺️'} ${areaMeta?.themeLabel || '미지의 지역'}`;
...
const party = getHeroParty(active);
const unit = getWorldUnitMembers(active);
const canAct = active && !active.acted && !active.down && !state.gameOver && !state.victory && !state.combat && unit.every(h => !h.acted && !h.down);
...
const maxX = mapLeft + mapW - dieSize * 0.88;
...
modalContent.innerHTML = `<div class="combat-item-sheet"><div class="status-kicker">COMBAT ITEM</div><h3>🎒 ${hero.name}의 아이템</h3><p>소비 아이템 사용은 <strong>이번 전투 행동 1회</strong>를 사용해. 사용이 끝나면 다음 영웅 또는 몬스터 턴으로 넘어가.</p><div class="combat-item-grid">${items.map(({item,index}) => `<button type="button" class="transfer-item-row" data-combat-item="${index}"><span>${item.icon}</span><strong>${item.name}</strong><small>${item.desc}</small></button>`).join('')}</div></div>`;
modalCloseBtn.textContent = '취소';
...
if (c.isBoss && c.node?.monsterId !== 'dragon' && !state.defeatedBosses.has(c.node.id)) {
...
<p>지역 입구를 밟았다. ${state.moveRemaining > 0 ? '이동하지 않으면 이 칸에 남아서 남은 이동을 계속해.' : '이동하지 않으면 이 입구에 머물고 이번 턴을 끝내.'}</p>
</div>
</div>
...
function unitNeedsVillageRecovery(unit) {
return (unit || []).some(member => {
const hpMissing = Number(member.currentHp || 0) < Number(member.hp || 0);
const manaMissing = member.currentMana !== null && member.currentMana !== undefined && member.currentMana < maxMana(member);
return hpMissing || manaMissing;
});
}
function askVillagePassChoice(hero, node, unit) {
return new Promise(resolve => {
modalCloseAction = null;
modal.classList.remove('hero-status-modal','party-manage-modal','item-transfer-modal','combat-item-modal','shop-modal');
modalCloseBtn.hidden = true;
const status = (unit || []).map(member => {
const mana = member.currentMana !== null && member.currentMana !== undefined
? ` · 🔵 ${member.currentMana}/${maxMana(member)}` : '';
return `${member.icon} ${member.name} · ❤️ ${member.currentHp}/${member.hp}${mana}`;
}).join('<br>');
modalContent.innerHTML = `
<div class="event-sheet village-pass-sheet">
<div class="status-kicker">VILLAGE STOP</div>
<div class="event-card-head"><span class="event-card-icon">🏠</span><div>
<h3>${node.name}에 들를까?</h3>
<p>회복하면 남은 MOVE ${state.moveRemaining}칸을 포기하고 이번 턴을 여기서 끝내.</p>
</div></div>
<div class="village-pass-status" style="margin:10px 0 14px;padding:9px 10px;border:1px solid #654d31;background:#1b1510;color:#ccb98f;font-size:10px;line-height:1.7">${status}</div>
<div class="equip-confirm-actions">
<button type="button" class="pixel-btn primary" data-village-rest>🏠 들러서 회복</button>
<button type="button" class="pixel-btn" data-village-pass>➡️ 지나간다</button>
</div>
</div>`;
modal.classList.remove('hidden');
const finish = choice => {
...
modalContent.querySelector('[data-village-rest]')?.addEventListener('click', () => finish('rest'), {once:true});
modalContent.querySelector('[data-village-pass]')?.addEventListener('click', () => finish('pass'), {once:true});
});
}
function recoverUnitAtVillage(unit) {
(unit || []).forEach(member => {
member.currentHp = member.hp;
member.down = false;
member.reviveRound = null;
if (member.currentMana !== null && member.currentMana !== undefined) member.currentMana = maxMana(member);
});
}
...
if (node.type === '마을' && state.moveRemaining > 0 && unitNeedsVillageRecovery(unit)) {
await revealLandedNode(nodeId);
const choice = await askVillagePassChoice(hero,node,unit);
if (choice === 'rest') {
const lostSteps = state.moveRemaining;
recoverUnitAtVillage(unit);
log(`🏠 <strong>${node.name}</strong>에 들러 회복 · 남은 이동 ${lostSteps}칸 소멸.`);
clearPlannedMoveState();
renderAll();
...
switch (node.type) {
case '마을':
recoverUnitAtVillage(unitMembers);
...
function canUseWorldConsumable(hero, item) {
if (!hero || !item || item.type !== 'consumable' || item.effect === 'autoRevive') return false;
if (!canUseWorldPrepActions()) return false;
const active = getActiveHero();
if (!getWorldUnitMembers(active).some(h => h.id === hero.id)) return false;
if (item.effect === 'heal') return hero.currentHp < hero.hp;
if (item.effect === 'mana') return hero.currentMana !== null && hero.currentMana < maxMana(hero);
return false;
}
function useWorldConsumable(hero, inventoryIndex) {
if (!hero || !canUseWorldPrepActions()) return false;
const active = getActiveHero();
if (!getWorldUnitMembers(active).some(h => h.id === hero.id)) return false;
const inv = heroInventory(hero);
const itemId = inv[inventoryIndex];
const item = window.getItemCard?.(itemId);
if (!canUseWorldConsumable(hero, item)) return false;
inv.splice(inventoryIndex, 1);
if (item.effect === 'heal') {
const before = hero.currentHp;
hero.currentHp = Math.min(hero.hp, hero.currentHp + Number(item.value || 0));
log(`${item.icon} ${hero.icon} <strong>${hero.name}</strong> · ${item.name} 사용 → HP +${hero.currentHp - before}`);
} else if (item.effect === 'mana') {
const before = hero.currentMana;
hero.currentMana = Math.min(maxMana(hero), hero.currentMana + Number(item.value || 0));
log(`🔵 ${hero.icon} <strong>${hero.name}</strong> · ${item.name} 사용 → MANA +${hero.currentMana - before}`);
} else {
inv.splice(inventoryIndex, 0, itemId);
return false;
}
renderAll();
openHeroStatus(hero);
...
modalContent.querySelectorAll('[data-use-index]').forEach(btn => btn.addEventListener('click',()=>{
